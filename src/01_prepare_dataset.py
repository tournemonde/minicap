...
def load_audio_sample(file_name):
    ...
    # sample_fs = sample.frame_rate
    sample = librosa.load(file_name, sr=48000)
    sample_fs = sample[1]

    # audio_data = np.array(sample.get_array_of_samples())
    norm_audio_data = sample[0] / (2**15)
    # time_wav = np.arange(0, len(norm_audio_data)) / sample_fs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default=debug_prefix + "data/03_primary/2021-08-09_testing_lib/", help='images output folder')
